...
- 在选项界面加入规则编辑器入口（仅单人游戏可用）

### 版本 0.3.7

- 在创建规则时，如果未提供参数，将显示创建该项目所需的参数列表和功能描述。

### 版本 0.3.6

流程图可视化编辑器

- 在选项界面加入流程图编辑器入口，支持可视化节点图编辑，包含节点拖拽、连线展示、撤销/重做等功能（仅单人游戏可用）

### 版本 0.3.5

自动加载功能

- 新增「自动加载规则文件」和「自动加载流程图文件」配置项，开启后服务端启动或进入新存档时会自动加载 config 目录下的所有 .rule 和 .flow 文件。
- 新增 PlayerJoinEventNode 流程图节点和 PlayerJoinEvent 规则事件。

### 版本 0.3.4

过度版本

- 代码由 Fabric Mapping 迁移到 Mojang 官方 Mapping.
...
- 游戏内全新自定义规则系统
  - 使用 `/f rule` 命令创建、编辑和管理自定义规则
  - 将自定义规则保存到 `.rule` 文件
  - 基于事件、条件、执行三个连续步骤的流水线

### 版本 0.3.2

大型 Bug 修复

- 修复了广播玩家消息时，在 TEAM 消息类型下由于 switch 语句缺少 break 导致错误触发 SELF 分支的问题
- 修复了二元运算节点中，由于错误的将输入强制转化为小写后判断，导致无法触发 String 字符串操作分支的问题
- 修复了在获取方块节点中，由于对输入进行四舍五入而不是向下取整，导致与原版游戏行为不一致的问题
- 修复了在播放 nbs 歌曲时，如果使用目标选择器同时向多个玩家播放，由于所有玩家共享一个音乐实例对象，导致后续调整播放速度时会错误的在所有玩家之间同步的问题
- 从 flow 文件中读取流程图时新增了大量的数值检查
- 修复了在判断生物群系的变化情况时，如果其它模组添加的生物群系没有注册，会抛出 NullPointerException 的问题
- 修复了在使用命令 Tab 自动补全时，网络线程和主线程之间的线程竞争问题
- 修复了部分情况下，从网络或者文件读取数据时资源未能正确关闭的问题
- 现在会阻止玩家创建名称为全空格的流程图
- 修复了在延迟执行流程图时，如果初始未提供任何变量，后续试图添加新的变量时会被丢弃的问题
- 修复了 Markdown 语法高亮无法正确处理 u00a7 字符的问题
- 现在流程图执行过程中抛出的由于用户自身导致的异常不会被日志记录
- 修复了控制台使用 f say 命令时使用有关玩家的占位符会抛出 NullPointerException 的问题
