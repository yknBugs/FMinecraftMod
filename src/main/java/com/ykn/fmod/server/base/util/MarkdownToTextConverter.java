        ...
        for (int i = 0; i < tokenCount; i++) {
            ...
            if (type.contains(MarkdownUnit.MATH)) {
                text = text.formatted(Formatting.DARK_RED).styled(s -> s
                    ...
                    .withHoverEvent(new HoverEvent(
                        ...
                        Util.parseTranslateableText("fmod.misc.copyto")
                    ))
                );
            ...
            if (type.contains(MarkdownUnit.HYPERLINK)) {
                ...
                text = text.styled(s -> s
                    ...
                    .withHoverEvent(new HoverEvent(
                        HoverEvent.Action.SHOW_TEXT,
                        Util.parseTranslateableText("fmod.misc.openurl", hint.get(type.indexOf(MarkdownUnit.HYPERLINK)))
                    ))
                );
            }
            result.append(text);
